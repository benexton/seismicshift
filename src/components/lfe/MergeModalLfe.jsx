...
function display(key, value) {
  if (key === 'observation_types') return observationTypesLabel(value);
...
// Arrays (observation_types) compare by sorted content, not reference/order,
// so the same set ticked in a different order isn't flagged as a clash.
const norm = (x) => {
  if (Array.isArray(x)) return [...x].sort().join(',');
 ...
 * scoped to the source's event since site numbers restart per event. Metadata
 * clashes are one-or-the-other; media, screenshots, links, and notes are
 * individually selectable on BOTH sides: tick source items to carry them
 ...
 */
export default function MergeModalLfe({ source, reviewer, candidates = [], onClose, onMerged }) {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabaseLfe.from('record_attachments')
  ...
  async function loadTarget(siteId) {
    ...
    const { data, error } = await supabaseLfe
      .from('triage_records').select(LFE_RECORD_COLUMNS)
      .eq('event_id', source.event_id).eq('site_id', n).is('merged_into', null).maybeSingle();
    if (error) return setErr(error.message);
    if (!data) return setErr(`No active record with Site #${n} in this event. It may not exist or may already have been merged.`);
    if (data.id === source.id) return setErr('That is the same record you are merging from.');
    setTarget(data);
    const { data: atts } = await supabaseLfe.from('record_attachments')
  ...
  async function confirmMerge() {
    ...
    setBusy(true); setErr('');
    try {
      // 1. remove any un-ticked existing attachments on the target
      const delIds = targetItems.filter((i) => i.attachmentId && !keep[i.key]).map((i) => i.attachmentId);
      if (delIds.length) {
        const del = await supabaseLfe.from('record_attachments').delete().in('id', delIds);
        if (del.error) {
          setBusy(false);
          return setErr('Could not remove existing attachments on the kept record. Either re-tick them, or check permissions. Nothing has been changed.');
        }
      }
      ...
      if (Object.keys(patch).length) {
        const up = await supabaseLfe.from('triage_records').update(patch).eq('id', target.id);
      ...
      if (rows.length) {
        const ins = await supabaseLfe.from('record_attachments').insert(rows);
        if (ins.error) throw ins.error;
      }

      // 4. mark the source merged (hidden but preserved)
      const mk = await supabaseLfe.from('triage_records')
