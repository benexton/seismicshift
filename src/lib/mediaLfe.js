import { supabaseLfe, LFE_MEDIA_BUCKET } from './supabaseLfe.js';

// Upload an image to the LFE media bucket and return its public URL.
export async function uploadImage(file, prefix = '') {
  const safe = file.name.replace(/[^\w.\-]/g, '_');
  const path = `${prefix}${Date.now()}_${Math.random().toString(36).slice(2, 7)}_${safe}`;
  const up = await supabaseLfe.storage.from(LFE_MEDIA_BUCKET).upload(path, file);
  if (up.error) throw up.error;
  return supabaseLfe.storage.from(LFE_MEDIA_BUCKET).getPublicUrl(path).data.publicUrl;
}

// Upload any file (PDF, image, etc.) to the LFE media bucket; returns its public URL.
